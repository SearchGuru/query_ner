...
def load_sequence_raw_data_x_y(path="./raw_data.tsv"):
    X = []
    Y = []
    f = open(path, "r")
    for l in f:
        ...
        x = fields[0]
        x_words = x.split(' ')
        y_classes = fields[1].split(' ')
        if (len(x_words) != len(y_classes)):
            continue        
        X.append(x)
        Y.append(y_classes)
    f.close()
...
def load_sequence_data_x_y(path="./raw_data.tsv", className2Id=None):
    X = []
    Y = []
    f = open(path, "r")
    for l in f:
...
def load_raw_data_termx(path="./raw_data.tsv", y_shift=-1, seed=1337, vocabs=None, add_reverse=False):
    X = []
    X2 = []
    Y = []
    f = open(path, "r")
